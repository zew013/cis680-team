...
class SAM(torch.nn.Module):
    def __init__(self, model_cfg='vit_b', project_special = None, img_size = None, **kwargs):
        ...
        if not os.path.exists(checkpoint):
            ...
            with open(checkpoint, "wb") as f:
                f.write(r.content)
        if project_special is None:
            sam: Sam = sam_model_registry[model_cfg](checkpoint=checkpoint)

        else:
            sam: Sam = project_special

        # self.project_special = True
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.img_size:
            interpolate_size = (self.img_size, self.img_size)
        else:
            interpolate_size = (1024, 1024)
        with torch.no_grad():
            x = torch.nn.functional.interpolate(x, size=interpolate_size, mode="bilinear")
